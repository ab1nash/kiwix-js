...
define(['q', 'uiUtil'], function(Q, uiUtil) {
    ...
    function setItem(key, contents, callback) {
        ...
        if (/cacheAssets=false\b/i.test(document.cookie) || !regexpKeyTypes.test(key)) {
            callback(-1);
            return;
        }
        ...
        document.getElementById('cachingCSS').innerHTML = 'Caching ' + shortTitle + '...';
        if (/^localStorage/.test(assetsCache.capability)) {
        ...
        getItem(key, function(result) {
            ...
            if (regexpKeyTypes.test(key)) {
                if (!/\.css$|\.js$/.test(key)) {
                    ...
                    document.getElementById('cachingCSS').style.display = 'block';
                }
            }
